...
export default function CampaignCard({ data, onAction }) {
  if (!data) return null;
    ...
    <div className="bg-surface-container-lowest rounded-xl border border-outline-variant overflow-hidden my-2 max-w-sm">
      <div className="px-4 py-3 bg-surface-container border-b border-outline-variant flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-label-md text-label-md text-on-surface-variant">Campaign</span>
        </div>
      ...
      <div className="p-4 space-y-3">
        ...
        <p className="font-body-md text-body-md text-on-surface font-semibold">{data.campaignName || 'Untitled Campaign'}</p>
        {data.campaignDescription && (
          <p className="font-label-md text-label-md text-outline">{data.campaignDescription}</p>
        )}
        ...
        {onAction && (
          <div className="flex gap-2 pt-1">
            <button
              onClick={() => onAction('view_campaign', data.id)}
            ...
            <button
              onClick={() => onAction('clone_campaign', data.id)}
